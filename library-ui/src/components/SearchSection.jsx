...
export default function SearchSection() {
  ...
  // Basic front-end filters mapped to DSpace metadata fields
  const filters = [
  ...
  const handleSearch = async (filterLabel = "All") => {
    ...
    try {
      ...
      const data = await searchPublications(query, apiField);
      ...
      <div className="card-content">
        ...
        <div className="filter-tags flex gap-2 mb-4 flex-wrap">
          {filters.map((f) => (
            <span
              key={f.label}
              className={`filter-tag px-3 py-1 rounded-full border cursor-pointer ${
...
// Utility function for API query
export async function searchPublications(query, filterField = null) {
  if (!query.trim()) return [];

  // Build query URL dynamically using filter parameter
  const base = "http://10.120.4.59:8080/server/api/discover/search/objects";

  // Example: filter query for title/author/year
